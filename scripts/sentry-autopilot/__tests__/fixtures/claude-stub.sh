#!/usr/bin/env bash
# Test stub for the Claude Code CLI (`claude`). Records its invocation and
# writes a synthetic outcome.json so session-supervisor.sh can complete a
# happy-path run without the real binary installed. Mirrors cursor-agent-stub.sh.
# Driven by env vars:
...

{
  ...
  echo "AUTOPILOT_CLAUDE_MODEL=${AUTOPILOT_CLAUDE_MODEL:-<unset>}"
