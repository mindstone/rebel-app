...
// MUST run before any fs operation — see docs/plans/260428_graceful_fs_emfile_fix.md
// Imports are hoisted in ESM; this top-level await runs after imports but
// before any subsequent fs op in this module. graceful-fs mutates the fs
// module object's methods, which the default `fs` import dereferences at call
// time, so the patch is effective for `fs.readFileSync(...)` etc. below.
if (process.env.REBEL_DISABLE_GRACEFUL_FS !== '1') {
  try {
    ...
    if (process.env.REBEL_DEBUG_BOOTSTRAP === '1') console.warn('[installGracefulFs] failed:', e);
  }
}
