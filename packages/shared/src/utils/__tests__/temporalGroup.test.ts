...
import { getTemporalGroup, getScheduleDueBy, groupByTemporal, computeTemporalBoundaries, TEMPORAL_GROUP_ORDER, TEMPORAL_GROUP_META } from '../../utils/temporalGroup';
import type { ConcreteTemporalGroup } from '../../utils/temporalGroup';
...
describe('getTemporalGroup', () => {
  ...
  it('explicit dueBy moves a fresh draft out of Today', () => {
    const threeDaysAgo = new Date(2026, 2, 7, 10, 0, 0).getTime();
    const friday = new Date(2026, 2, 13, 10, 0, 0).getTime();
    expect(getTemporalGroup(makeItem({ addedAt: threeDaysAgo, draft: 'Hello team,', dueBy: friday }))).toBe('due-this-week');
  });

  it('explicit dueBy wins over calendar matching', () => {
    const friday = new Date(2026, 2, 13, 10, 0, 0).getTime();
    expect(
      getTemporalGroup(
        makeItem({ addedAt: Date.now(), dueBy: friday }),
        undefined,
        { calendarMatchedIds: new Set(['matched']), itemId: 'matched' },
      ),
    ).toBe('due-this-week');
  });
...
describe('getScheduleDueBy', () => {
  ...
  const allGroups: ConcreteTemporalGroup[] = ['due-today', 'due-this-week', 'upcoming'];

  describe('roundtrip with getTemporalGroup', () => {
    it.each(allGroups)('%s → getScheduleDueBy → getTemporalGroup roundtrips', (group) => {
      const b = computeTemporalBoundaries();
      const dueBy = getScheduleDueBy(group, b);
      const result = getTemporalGroup(makeItem({ dueBy, addedAt: Date.now() }), b);
      expect(result).toBe(group);
    });
  });

  it('due-today returns exactly todayEndMs - 1', () => {
    const b = computeTemporalBoundaries();
    expect(getScheduleDueBy('due-today', b)).toBe(b.todayEndMs - 1);
  });

  it('due-this-week returns exactly weekEndMs - 1', () => {
    const b = computeTemporalBoundaries();
    expect(getScheduleDueBy('due-this-week', b)).toBe(b.weekEndMs - 1);
  });

  it('upcoming returns exactly weekEndMs', () => {
    const b = computeTemporalBoundaries();
    expect(getScheduleDueBy('upcoming', b)).toBe(b.weekEndMs);
  });

  it('old item scheduled to Today lands in Today (concrete timestamp beats addedAt fallback)', () => {
    const twoWeeksAgo = new Date(2026, 1, 24, 10, 0, 0).getTime();
    const b = computeTemporalBoundaries();
    const dueBy = getScheduleDueBy('due-today', b);
    const result = getTemporalGroup(makeItem({ dueBy, addedAt: twoWeeksAgo }), b);
    expect(result).toBe('due-today');
  });

  it('uses provided boundaries instead of computing fresh ones', () => {
    const customBoundaries = {
      nowMs: new Date(2026, 5, 15, 12, 0, 0).getTime(),
      todayStartMs: new Date(2026, 5, 15, 0, 0, 0).getTime(),
      todayEndMs: new Date(2026, 5, 16, 0, 0, 0).getTime(),
      weekEndMs: new Date(2026, 5, 22, 0, 0, 0).getTime(),
      weekStartMs: new Date(2026, 5, 9, 0, 0, 0).getTime(),
    };
    expect(getScheduleDueBy('due-today', customBoundaries)).toBe(customBoundaries.todayEndMs - 1);
    expect(getScheduleDueBy('due-this-week', customBoundaries)).toBe(customBoundaries.weekEndMs - 1);
    expect(getScheduleDueBy('upcoming', customBoundaries)).toBe(customBoundaries.weekEndMs);
  });

  it('computes boundaries from current time when none provided', () => {
    const b = computeTemporalBoundaries();
    expect(getScheduleDueBy('due-today')).toBe(b.todayEndMs - 1);
    expect(getScheduleDueBy('due-this-week')).toBe(b.weekEndMs - 1);
    expect(getScheduleDueBy('upcoming')).toBe(b.weekEndMs);
  });
});
