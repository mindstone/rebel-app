/**
 * CloudContentStore — Node.js HTTP server implementation of the `ContentStore`
 * boundary. Mirrors `DesktopContentStore` against the cloud-service data
 * directory.
 *
...
const log = createScopedLogger({ service: 'contentStoreCloud' });
...
export interface ContentStoreFs {
  ...
  // Optional durability hooks; see DesktopContentStore for the contract.
  fsyncFile?(p: string): Promise<void>;
  fsyncDir?(p: string): Promise<void>;
}

interface CloudContentStoreOptions {
  baseDir?: string;
  fs?: ContentStoreFs;
}
...
export class CloudContentStore implements ContentStore {
  private readonly baseDirOverride?: string;
  private readonly fs: ContentStoreFs;

  constructor(options: CloudContentStoreOptions = {}) {
  ...
  async writeContent(args: {
    ...
    // Cloud-side manifest parity with DesktopContentStore (Stage B1a § MEDIUM #1):
    // tracks mimeType + byteSize so readContent can serve the correct content
    // type even when the desktop manifest hasn't been replicated yet.
    await this.updateManifestEntry({
      sessionId,
      contentId,
      mimeType,
      byteSize: bytes.byteLength,
  ...
  private async writeFileAtomic(
    ...
    // fsync the tmp file before publishing the name. See Stage B1a § HIGH #6.
  ...
  private async updateManifestEntry(args: {
    sessionId: string;
    contentId: string;
    mimeType: string;
    byteSize: number;
    ...
    try {
      ...
      for (const [contentId, value] of Object.entries(parsed)) {
        if (
          ...
          manifest[contentId] = {
            uploadStatus: value.uploadStatus === 'pending' || value.uploadStatus === 'missing'
              ? value.uploadStatus
              : 'uploaded',
      ...
      if (code !== 'ENOENT') {
        log.warn(
          { ...redacted, op: args.op, errCode: code },
    ...
    manifest[args.contentId] = {
      ...
      ...(manifest[args.contentId]?.firstQueuedAt !== undefined
        ? { firstQueuedAt: manifest[args.contentId].firstQueuedAt }
        : {}),
    };
