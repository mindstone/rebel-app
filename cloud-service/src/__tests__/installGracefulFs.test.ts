/**
 * Smoke tests for the cloud-service graceful-fs boot-time install (Stage 2).
 *
 * Mirrors `src/main/startup/__tests__/installGracefulFs.test.ts` (desktop)
 * for the cloud surface. Three tests:
 *   1. Subprocess EMFILE injection on callback `fs.open` — proves graceful-fs
 *      patches the fs module and queues+retries EMFILE callbacks.
 *   2. `fs/promises` reality check — pins the observed behaviour that
 *      graceful-fs 4.2.11 does NOT patch `node:fs/promises` named imports.
 *   3. Source-order guard — asserts the leaf module is the first non-comment
 *      import of `cloud-service/src/server.ts`, so no later refactor
 *      accidentally moves it below an fs-touching import.
 *
...
const SERVER_PATH = path.join(REPO_ROOT, 'cloud-service', 'src', 'server.ts');
const CLOUD_INSTALL_LEAF_PATH = path.join(REPO_ROOT, 'cloud-service', 'src', 'installGracefulFs.ts');
...
describe('installGracefulFs (cloud-service)', () => {
  ...
  it('documents fs/promises retry behaviour (the named-import surface)', () => {
    ...
    // and the test fails — at which point we'd revisit the cloud's hot-path
    // hardening scope.)
    expect(result.stdout.trim()).toBe('OUTCOME=resolved');
  });

  // ─── Test 3: source-order guard ──────────────────────────────────────────
  // The leaf module must be the very first non-comment import of
  // `cloud-service/src/server.ts`. A later refactor that moves it below
  // another import would silently regress the patch. Cheap, deterministic,
  // run as part of validate:fast via the unit-test suite.
  it('keeps installGracefulFs as the first import in cloud-service/src/server.ts', () => {
    const source = fs.readFileSync(SERVER_PATH, 'utf8');
    const firstImport = findFirstImport(source);
    expect(firstImport, 'no import statements found in server.ts').not.toBeNull();
    expect(firstImport).toBe('./installGracefulFs');
  });

  it('routes cloud startup install through @core/startup/installGracefulFs', () => {
    const source = fs.readFileSync(CLOUD_INSTALL_LEAF_PATH, 'utf8');
    const firstImport = findFirstImport(source);
    expect(firstImport, 'no import statements found in cloud install leaf').not.toBeNull();
