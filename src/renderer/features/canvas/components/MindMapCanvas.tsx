...
const MIND_ELIXIR_CSS_VARS = {
  light: {
    ...
    '--main-color': '#4f46e5',
    '--main-bgcolor': '#f1f5f9',
    '--main-bgcolor-transparent': 'rgba(241, 245, 249, 0.95)',
    '--main-radius': '8px',
    '--main-gap-x': '40px',
    '--main-gap-y': '20px',
    '--node-gap-x': '24px',
  ...
  dark: {
    ...
    '--selected': '#a5b4fc',
    '--root-color': '#f8fafc',
    '--root-bgcolor': 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)',
    '--root-radius': '12px',
    '--root-border-color': '#818cf8',
    '--main-color': '#c7d2fe',
    '--main-bgcolor': '#1e1b4b',
    '--main-bgcolor-transparent': 'rgba(30, 27, 75, 0.95)',
    '--main-radius': '8px',
    '--main-gap-x': '40px',
    '--main-gap-y': '20px',
    '--node-gap-x': '24px',
    ...
    '--topic-padding': '8px 16px',
    '--map-padding': '100px',
    '--panel-bgcolor': '#1e1b4b',
    '--panel-color': '#e2e8f0',
    '--panel-border-color': '#312e81',
  },
};
...
interface MindMapCanvasProps {
  ...
  direction?: number;
  theme?: 'light' | 'dark';
  onReady?: (instance: MindElixirInstance) => void;
}

/** Flatten mind map tree to semantic text for AI */
export function extractSemanticText(data: MindElixirData): string {
  const lines: string[] = [];
  
  const traverse = (node: NodeObj, depth: number = 0) => {
    const indent = '  '.repeat(depth);
    const bullet = depth === 0 ? '' : '- ';
    lines.push(`${indent}${bullet}${node.topic}`);
...
function countMindMapNodes(node?: NodeObj): number {
  if (!node) return 0;
  return 1 + (node.children?.reduce((count, child) => count + countMindMapNodes(child), 0) ?? 0);
}

export function MindMapCanvas({ initialData, direction = MindElixir.SIDE, theme = 'dark', onReady }: MindMapCanvasProps) {
  ...
  useEffect(() => {
    ...
    if (container.offsetWidth === 0 || container.offsetHeight === 0) {
      const timer = setTimeout(() => {
        // Re-trigger effect by forcing update
    ...
    const mind = new MindElixir({
      ...
      draggable: true,
      contextMenu: true,
      toolBar: false, // Disable built-in toolbar - clutters the UI
      keypress: true,
    });

    const data = initialData || MindElixir.new('Your idea');
    mind.init(data);
    tracking.mindMap.rendered(countMindMapNodes(data.nodeData));
    mindRef.current = mind;
    
    // Center after layout settles - need multiple frames for Mind Elixir to finish
    const centerView = () => {
      mind.toCenter();
    };
    // Track RAF IDs for cleanup cancellation
    rafIdsRef.current = [];
    const outerId = requestAnimationFrame(() => {
      const innerId = requestAnimationFrame(() => {
        // Guard: skip if instance was destroyed during the 2-frame wait
        if (mindRef.current) {
          centerView();
        }
      });
    ...
    // Also re-center when nodes change
    const bus = mind.bus as { addListener(event: string, fn: () => void): void; removeListener(event: string, fn: () => void): void } | undefined;
    bus?.addListener('operation', centerView);
    setIsInitialized(true);
    onReady?.(mind);

    return () => {
      // 1. Cancel pending RAF callbacks
      for (const id of rafIdsRef.current) {
        cancelAnimationFrame(id);
      }
      rafIdsRef.current = [];
      // 2. Remove bus listener BEFORE destroy (destroy nulls bus)
      bus?.removeListener('operation', centerView);
      // 3. Destroy instance — runs disposables, clears innerHTML, nulls internals
      mind.destroy();
      // 4. Null the ref
      mindRef.current = null;
    };
  }, [initialData, direction, onReady, isInitialized]);
  ...
  useEffect(() => {
    ...
    const container = containerRef.current;
    const vars = MIND_ELIXIR_CSS_VARS[theme];
